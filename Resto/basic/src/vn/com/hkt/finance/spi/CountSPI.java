...
public class CountSPI implements ICount {
    ...
    public int countDepartmentByEnterprise(Enterprise enterprise) {
        ...
        List<Department> list = departmentBN.selectAll();
        if (!list.isEmpty()) {
    ...
    public int countOperation(Enterprise enter, Department depar, Person person, Product product) {
        ...
        list = operationBN.selectAll();
        if (!list.isEmpty()) {
            for (Operation bean : list) {
//                if ((bean.getDepartmentIdActual()==(depar.getId())) &&
//                        (bean.getPersonIdActual()==(person.getId()))
//                        && (bean.getProductIdActual()==(product.getId()))) {
//                    S++;
//                }
            }
        }
    ...
    public int countOperation(Enterprise enter, String date) {
        ...
        if (!list.isEmpty()) {
            for (i = 0; i < list.size(); i++) {
                if (list.get(i).getEnterpriseIdActual()==(enter.getId())) {
                    S++;
                }
            }
        }
    ...
    @Override
    public int countOperation(Enterprise enter, String classificationName, String date) {
        int S = 0;
        int i;
        List<Operation> list = operationBN.filterOperationByDate(date);
        if (!list.isEmpty()) {
            for (i = 0; i < list.size(); i++) {
                Classification cl= new ClassificationBN().getById(list.get(i).getClassificationIdActual());
                if ((list.get(i).getEnterpriseIdActual()==(enter.getId()))
